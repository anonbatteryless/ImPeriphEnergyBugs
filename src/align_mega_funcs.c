...
void align_mega_init(int mat_dim) {
	uint16_t count = mat_dim;
...
void align_mega(int mat_dim,mat_t *src1, mat_t *src2, mat_t *dest, mat_t *inter){
...
void align_mega_check(int mat_dim) {
#ifdef CONSOLE
	uint16_t count = mat_dim;
...
void align_mega_data(float (*sense)(),int mat_dim) {
	uint16_t count = mat_dim;
  UNSAFE_ATOMIC_START;
	for(uint16_t i = 0; i < count; i++) {
...
void align_mega_short_data(float (*sense)(),uint16_t sample_len,int mat_dim) {
	uint16_t count = mat_dim;
	static uint16_t i = 0;
  //UNSAFE_ATOMIC_START;
