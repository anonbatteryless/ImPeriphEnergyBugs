...
void dconv(int mat_dim, mat_t *src, mat_t *dest, mat_t *inter, mat_t *filter) {
...
void dconv_check(void) {
	...
	uint16_t rows2 = MAT_GET_DIM(b2,0);
	uint16_t cols2 = MAT_GET_DIM(b2,1);
...
void dconv_short_data(float(*sense)(),uint16_t sample_len) {
	uint16_t rows = MAT_GET_DIM((&mat_a_dense), 0);
	uint16_t cols = MAT_GET_DIM((&mat_a_dense), 1);
#ifndef HPVLP
	printf("Starting:\r\n");
