// Big case study for testing with pudu
// We pull in modes from an interrupt and things go from there.
#include <msp430.h>
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
#define ALL_ACCEL 0 //Run back-to-back iterations of accel 
#define ALL_PARTICLE 1
#define MIX 2

volatile __nv unsigned hill_mode = ALL_ACCEL;
...
void inline merged_func(unsigned param) {
  ...
  if (use_accel) {
    for (int i = 0; i < 100; i++){//We'll run accel for same time as comp
      int16_t x,y,z;
      READ_ACCEL_PRE;
  ...
  UNSAFE_ATOMIC_END;
    ...
    prev_ps = cur_ps;
    cur_ps = tmp;
    if (hill_mode == ALL_ACCEL || hill_mode == MIX){
      use_accel = true;
    }
    else {
      use_accel = false;
...
__nv unsigned int mode_switch_cnt = 0;

int next_mode[10] = {0, 0, 1, 2, 1, 1, 0, 2, 2, 2, 0};
void port3_init();
...
int DRIVER main(void)
{
  ...
  for(int __i = 0; __i < 500; __i++) {
    BIT_FLIP(1,0);
    BIT_FLIP(1,0);
    if (P3IN & BIT5) {
      mode_switch_cnt++;
      if (mode_switch_cnt > 9) {
        mode_switch_cnt = 0;
      }
      hill_mode = next_mode[mode_switch_cnt];
      //Pulse p1.5
      BIT_FLIP(1,5);
      BIT_FLIP(1,5);
    }
...
//Can't use an interrupt on port 3 because that's our checkpoint routine...
